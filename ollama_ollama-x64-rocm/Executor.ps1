...
## Ollama ROCm is an addon image for ollama/ollama-x64, so it is tested by layering it over
...
## Note: on a test VM without an AMD GPU, Ollama falls back to CPU inference; the test verifies
## the addon merges cleanly and the application remains functional.

$image = "ollama/ollama-x64-rocm"
$app = "ollama/ollama-x64"
$using = "ollama/ollama-x64-rocm,ollama/ollama-model-gemma4:e4b"  # Addon under test + test model image
...
# Start the server detached with the ROCm addon and model image layered over the base image.
...
try {
    ...
    $logLines += "PASS: Server is up with ROCm addon. Version = $($version.version)"
