...
## The test starts the image (default startup file runs "ollama serve") with the gemma4:e4b
## model image layered in via --using, waits for the HTTP API to come up on localhost:11434,
...
$image = "ollama/ollama-x64"
$using = "ollama/ollama-model-gemma4:e4b"  # Provides the test model, so no model download is needed
...
# Start the server detached with the model image layered in.
# The image's default startup file runs "ollama serve".
TryTurboApp -image $image -using $using -extra $extra -detached $True
...
try {
    ...
    $logLines += "PASS: Server is up. Version = $($version.version)"
